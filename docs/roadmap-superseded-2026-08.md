...
>    consequence.
>
> 6. **AASA test prep is not the stated goal.** `curriculum-requirements.md` (removed — see PRIVACY.md)
>    optimizes heavily for the spring state test. The actual goal is
>    "summer-long, expandable coursework that keeps pace with him." the state
...
10. **Extend the content schema and split content out of the engine.** Add a `standards: ['<state-standard-code>', …]` array field to the day/quiz-item schema (items can span multiple standards), and move curriculum content out of the JS source into standalone data files, separate from the engine (`curric` §9 item 1's "tag content by state standard code," and the architecture-implication note on splitting content into data files with scripted validation instead of hand-authoring). Keep the validation tooling lightweight at this stage — a script checking every question has `explain`+`hint`, denominators stay in `{2,3,4,5,6,8,10,12,100}` per `curric` §3.6, etc. — and expand it as Stage 1/2 reveal what's worth checking.
