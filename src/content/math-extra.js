...
export const MATH_EXTRA = [
  /* ---- m15 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'Support rises from 4% to 6%. Which statement is TRUE?', choices: ['it rose by 2 percentage points', 'it rose 50 percentage points', 'it doubled', 'it rose 2%'], answer: 0, hint: 'A percentage point is the plain gap between the two percentages.', explain: 'Two percentage points — which is also a 50% increase. Both are true; "it rose 2%" is the one that is not.' },
      { type: 'mc', prompt: 'A price rises from $80 to $100, then falls from $100 back to $80. The percentages are:', choices: ['a 20% rise then a 25% fall', 'a 25% rise then a 20% fall', 'both 20%', 'both 25%'], answer: 1, hint: 'Divide by the starting value each time.', explain: '20 ÷ 80 = 25% up; 20 ÷ 100 = 20% down. The same twenty dollars, two different percentages.' },
    ],
  },
  ...
  /* ---- m16 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'The mean of a group is much higher than the median. This usually means:', choices: ['the group is very large', 'most values are above average', 'the data was recorded wrongly', 'a few values sit far above the rest'], answer: 3, hint: 'Which average gets dragged by one enormous value?', explain: 'The mean uses every value, so one billionaire pulls it upward. The median just steps to the middle and barely notices.' },
    ],
  },
  ...
  /* ---- m18 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'Two coins are flipped. The probability of exactly one head is:', choices: ['1/4', '3/4', '1/2, because HT and TH both count', '1/3, since there are three outcomes'], answer: 2, hint: 'List all four results before you decide.', explain: 'HH, HT, TH, TT — two of the four have exactly one head, so 2/4 = 1/2.' },
    ],
  },
  ...
  /* ---- m19 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'A mouse must eat almost constantly because it:', choices: ['moves far too quickly to stop eating', 'cannot store any food inside its body', 'has a very small stomach for its size', 'has a large surface area for its volume'], answer: 3, hint: 'Where does a small warm animal lose heat?', explain: 'Lots of surface for very little body, so heat escapes fast and has to be replaced.' },
      { type: 'mc', prompt: 'An insect scaled up to the size of a dog would collapse mainly because:', choices: ['weight grows faster than leg strength', 'its legs would be far too short to walk', 'it could not find enough food to eat', 'its shell would be the wrong thickness'], answer: 0, hint: 'Compare how area grows with how volume grows.', explain: 'Strength depends on cross-sectional area, which squares. Weight depends on volume, which cubes. Doubling in size makes it eight times heavier but only four times stronger.' },
    ],
  },
  ...
  /* ---- mr3 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'A 30% cut followed by a 30% rise leaves the price:', choices: ['exactly where it started', 'below where it started', 'above where it started', 'impossible to tell'], answer: 1, hint: 'The rise is calculated from a smaller number than the cut was.', explain: '$100 → $70 → $91. The base shrank, so the same percentage returns less.' },
      { type: 'mc', prompt: 'A correctly calculated average can still:', choices: ['only be wrong if the maths is wrong', 'never mislead anyone', 'describe nobody in the group', 'always name a real member of the group'], answer: 2, hint: 'Think of the average family with 2.4 children.', explain: 'No family has 2.4 children. The number is arithmetically perfect and describes no actual household.' },
      ...
      { type: 'mc', prompt: 'After eight tails in a row, the chance the next flip is heads is:', choices: ['impossible to know', 'higher, because heads is due', 'lower', 'one half'], answer: 3, hint: 'What does independent mean?', explain: 'One half. The rarity of a long run is spent before it happens, not owed afterward.' },
      ...
      { type: 'mc', prompt: 'The single question underneath all five days is:', choices: ['is the arithmetic actually correct', 'compared to what, and did it stay still', 'who calculated this number first', 'is this number large or small'], answer: 1, hint: 'Each trap involved a moving baseline.', explain: 'Every one of the five hid something changing that intuition assumed was fixed.' },
      { type: 'mc', prompt: 'Support rises from 3% to 6%. Which description is honest?', choices: ['only “a 100% increase” is fair', 'only “3 percentage points” is fair', 'both — they measure different things', 'neither describes it honestly'], answer: 2, hint: 'One is the plain gap; one compares against the start.', explain: 'Both are true. One sounds trivial and one sounds enormous, and whoever reports it picks which impression you leave with.' },
    ],
  },
];
