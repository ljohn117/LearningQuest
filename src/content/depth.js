...
export const ELA_EXTRA = [
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: '"Unbreakable" breaks into:', choices: ['unbreak + able', 'un + break + able', 'unb + reak + able', 'un + breakable + e'], answer: 1, hint: 'Find the prefix, the root, then the suffix.', explain: 'un (not) + break + able (can be) — not able to be broken.' },
      ...
      { type: 'mc', prompt: 'If "bene" means good, "benefit" most likely involves:', choices: ['something old', 'something harmful', 'something good', 'something fast'], answer: 2, hint: 'Use the root even though you know the word already.', explain: 'Good. Roots let you predict meaning before you look anything up.' },
    ],
  },
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'A photograph taken during an event is a:', choices: ['summary', 'an opinion piece', 'secondary source', 'primary source'], answer: 3, hint: 'Was the camera there?', explain: 'Primary — it records the event directly.' },
      ...
      { type: 'mc', prompt: 'The most useful question about a source is:', choices: ['how would I check it', 'is it recent', 'is it popular', 'is it long'], answer: 0, hint: 'Which question leads somewhere?', explain: 'Checkability. It turns a judgement into an investigation.' },
      { type: 'mc', prompt: 'A company describing its own product is:', choices: ['always lying', 'a source with a motive worth knowing', 'a primary source only', 'never useful'], answer: 1, hint: 'Motive is not the same as dishonesty.', explain: 'It may be accurate, but it has reasons to emphasise some things. Know where to check.' },
    ],
  },
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: '"In my personal opinion I think it was actually quite good" is weak mainly because it:', choices: ['repeats the same idea several ways', 'uses no evidence', 'is in past tense', 'is too short'], answer: 0, hint: 'Count how many times it hedges.', explain: 'Opinion, think, actually and quite all do the same job. Pick one.' },
    ],
  },
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'Voice is like branding because both depend on:', choices: ['being loud', 'being formal', 'spending money', 'consistency people can recognise'], answer: 3, hint: 'What makes something recognisable?', explain: 'Keeping the same promise until people know it on sight.' },
    ],
  },
];
...
export const BIO_EXTRA = [
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'Evolution needs enormous time because:', choices: ['change is fast', 'tiny advantages take many generations to compound', 'fossils are rare', 'DNA is complicated'], answer: 1, hint: 'How big is a one percent advantage in one generation?', explain: 'Small effects need many repetitions — the same compounding as interest.' },
      { type: 'mc', prompt: 'A trait becomes more common when its owners:', choices: ['are larger', 'live longer only', 'leave more offspring', 'are stronger'], answer: 2, hint: 'What actually passes a trait on?', explain: 'Reproduction is the mechanism. Survival matters because it enables it.' },
    ],
  },
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'The correct order from smallest is:', choices: ['tissue, cell, system, organ', 'system, organ, cell, tissue', 'organ, tissue, cell, system', 'cell, tissue, organ, system'], answer: 3, hint: 'Start with the smallest living unit.', explain: 'Cell, tissue, organ, system — each built from the one before.' },
      ...
      { type: 'mc', prompt: 'The shared purpose of organ systems is best described as:', choices: ['keeping internal conditions steady', 'producing energy only', 'fighting disease only', 'growing larger'], answer: 0, hint: 'One word from Day 1.', explain: 'Homeostasis — holding conditions within a narrow band.' },
      { type: 'mc', prompt: 'A body resembles a well-built program because both:', choices: ['are written down', 'combine small parts each doing one job', 'never fail', 'are fast'], answer: 1, hint: 'Think about decomposition.', explain: 'Small specialised pieces combining into something larger.' },
    ],
  },
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'A virus cannot be clearly called alive because it:', choices: ['does not move', 'is too small', 'cannot reproduce without a host cell', 'has no DNA'], answer: 2, hint: 'Check it against the Day 1 list.', explain: 'It fails several checklist items, reproduction most obviously.' },
      ...
      { type: 'mc', prompt: 'Decomposers matter because they:', choices: ['eat live animals', 'make soil harder', 'produce oxygen', 'return nutrients so they can be reused'], answer: 3, hint: 'What would pile up without them?', explain: 'They close the loop and keep the cycle turning.' },
      { type: 'mc', prompt: 'Without decomposers a food web would be:', choices: ['a one-way line ending in waste', 'faster', 'unchanged', 'a cycle'], answer: 0, hint: 'Nothing would come back.', explain: 'Nutrients would lock up permanently in dead material.' },
    ],
  },
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'A hypothesis is useful only if:', choices: ['it is popular', 'some possible result could show it wrong', 'it is complicated', 'it is already known'], answer: 1, hint: 'What would count as evidence against it?', explain: 'It must be able to fail. Otherwise the test tells you nothing.' },
      ...
      { type: 'mc', prompt: 'The control group is there to:', choices: ['save time', 'make the experiment bigger', 'give a baseline for comparison', 'confirm the hypothesis'], answer: 2, hint: 'Compared against what?', explain: 'Without a baseline, a result has no meaning.' },
      { type: 'mc', prompt: 'Why is a hypothesis never called "proved"?', choices: ['proof is only for math', 'it is a translation error', 'scientists are cautious by habit', 'no number of confirming results rules out a future counterexample'], answer: 3, hint: 'Remember the swans.', explain: 'Examples support but never prove a universal claim — straight from Logic.' },
    ],
  },
];
