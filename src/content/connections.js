    ...
    days: [
      {
        ...
        quiz: [
          ...
          { type: 'mc', prompt: 'Binary, half-life and compound interest share which mechanism?', choices: ['repeated addition of a fixed amount', 'measuring against a starting value', 'random variation', 'repeated multiplication by a fixed factor'], answer: 3, hint: 'What does an exponent actually count?', explain: 'All three multiply by the same factor over and over — 2 for binary, one half for decay, 1.05 or so for interest.' },
        ],
      },
      {
        ...
        quiz: [
          ...
          { type: 'mc', prompt: 'An if-statement in code is structurally the same as:', choices: ['a variable', 'a conditional in logic', 'a definition', 'a counterexample'], answer: 1, hint: 'Both check something, then act.', explain: 'Same structure, different uniform.' },
          { type: 'mc', prompt: '“If it rains the game is cancelled. The game was cancelled, so it rained.” The flaw is that:', choices: ['it relies on far too few examples to work', 'the two premises given are simply false', 'it reverses the if-then — other things cancel games', 'it uses emotional language to persuade you'], answer: 2, hint: 'What else cancels a game?', explain: 'Rain guarantees cancellation; cancellation does not guarantee rain. Flu, a double booking, a broken pitch — reversing an if-then is a different claim.' },
          { type: 'mc', prompt: 'A conditional is broken when:', choices: ['the condition turns out to be false', 'both parts of it turn out to be true', 'nobody ever bothers to check either part', 'the condition is true but the result is not'], answer: 3, hint: 'There is only one failing case.', explain: 'Condition met, promised result absent. That is the only way.' },
        ],
      },
      {
        ...
        quiz: [
          ...
          { type: 'mc', prompt: 'A unit rate is a ratio where the second quantity is:', choices: ['one', 'always money', 'zero', 'the same as the first'], answer: 0, hint: 'Think about "per ONE item".', explain: 'Per one — that is what makes it a unit rate.' },
          { type: 'mc', prompt: 'A ratio comparing two DIFFERENT kinds of quantity, like dollars and hours, is:', choices: ['not a valid ratio, since units differ', 'a rate — perfectly valid and very useful', 'only valid inside scientific work', 'exactly the same as a percentage'], answer: 1, hint: 'Think about dollars per hour, or miles per gallon.', explain: 'Those are rates, and they are ratios doing their most useful work — miles per hour, price per gram, people per square mile.' },
      ...
      },
      {
        ...
        quiz: [
          { type: 'mc', prompt: 'What do fossil layers, text quotes and error messages have in common?', choices: ['opinions people happen to hold', 'statements that are always reliable', 'evidence left behind by something unseen', 'tools used only within science'], answer: 2, hint: 'What can none of them let you do directly?', explain: 'None of them let you observe the event — you reconstruct it from traces.' },
          { type: 'mc', prompt: 'Which best supports a claim about a character?', choices: ['stating the claim more confidently', 'saying how the passage made you feel', 'saying most readers would agree', 'quoting what the character said or did'], answer: 3, hint: 'What can the reader check for themselves?', explain: 'Evidence is something on the page that anyone can go and look at. A feeling is not checkable, however genuine it is.' },
          { type: 'mc', prompt: 'A lower rock layer is generally:', choices: ['older', 'younger', 'impossible to date', 'the same age'], answer: 0, hint: 'Which went down first?', explain: 'Older — layers pile up over time, so the deepest was laid down first.' },
          { type: 'mc', prompt: 'The best first move when a program breaks is to:', choices: ['start the whole thing over again', 'read what the error message says', 'rewrite the part you suspect', 'guess what probably went wrong'], answer: 1, hint: 'It already left you evidence.', explain: 'Read the evidence first. Same discipline as every other lane here.' },
        ],
      },
      {
        ...
        quiz: [
          { type: 'mc', prompt: 'A rolling ball slows and stops. The kinetic energy:', choices: ['was never there', 'turned into mass', 'became heat', 'was destroyed'], answer: 2, hint: 'What warms up when things rub?', explain: 'Friction converted it to heat. The total is unchanged.' },
          { type: 'mc', prompt: 'A conservation law states that:', choices: ['it applies only to energy, not matter', 'the amount always increases over time', 'nothing in the system ever moves', 'the total is unchanged, though it may move'], answer: 3, hint: 'Conserved does not mean frozen.', explain: 'Things move and change form freely; the total stays put. That is why “where did it go?” always has an answer.' },
          { type: 'mc', prompt: 'If your total changed during a calculation, the most useful conclusion is:', choices: ['you missed something', 'the law is wrong', 'start over from scratch', 'the total does not matter'], answer: 0, hint: 'Which conclusion helps you find the error?', explain: 'You missed something — and now you know to look, which is worth more than a wrong answer you trust.' },
          { type: 'mc', prompt: 'Water on Earth is:', choices: ['used up and then replaced by rainfall', 'conserved — the same water keeps cycling', 'created in the depths of the oceans', 'slowly leaking away into outer space'], answer: 1, hint: 'Where did the dinosaurs’ drinking water go?', explain: 'Nothing consumes it. Rain moves water, it does not manufacture it, which is why the glass you drank is genuinely ancient.' },
        ],
      },
      {
        ...
        quiz: [
          { type: 'mc', prompt: 'Knowing everything about hydrogen and oxygen separately would NOT tell you:', choices: ['their atomic numbers', 'their masses', 'that water is wet', 'their proton counts'], answer: 2, hint: 'Which property belongs to the combination?', explain: 'New properties appear when parts combine.' },
          { type: 'mc', prompt: 'Living things obey physics completely. Does that make biology unnecessary?', choices: ['yes, given enough computing power', 'yes — biology is just very slow physics', 'no, because life breaks physical laws', 'no — larger patterns appear regardless'], answer: 3, hint: 'Think about what a heartbeat is, described purely in atoms.', explain: 'Nothing violates physics, but predation, inheritance and immunity are real patterns you would never spot in a list of particle positions.' },
          { type: 'mc', prompt: 'The pattern shared by atoms, cells and ecosystems is:', choices: ['specialised parts combining into more', 'they are all forms of living matter', 'they all contain carbon compounds', 'they are all far too small to see'], answer: 0, hint: 'What is true at every scale?', explain: 'Parts with jobs, combining into a whole.' },
          { type: 'mc', prompt: 'Faced with something too large to picture, the useful move is to ask:', choices: ['who first discovered the thing', 'what are the parts, and what do they do', 'how far away from us is it', 'how old is it thought to be'], answer: 1, hint: 'The two questions from page one.', explain: 'Parts and their interaction — it works at every scale.' },
        ],
      },
      {
        ...
        quiz: [
          { type: 'mc', prompt: 'What do homeostasis, supply and demand, and checks and balances share?', choices: ['they all require people to run them', 'they never change once established', 'a disturbance triggers its own correction', 'they are all biological in nature'], answer: 2, hint: 'What happens when each is pushed?', explain: 'Each pushes back against being pushed. Same mechanism, three subjects.' },
          { type: 'mc', prompt: 'A system in balance is best described as one where:', choices: ['every force acting on it is zero', 'nothing at all is changing any more', 'change has stopped there permanently', 'changes happen constantly and cancel out'], answer: 3, hint: 'Think about a thermostat in a warm room.', explain: 'Your body temperature holds steady through constant activity, not stillness. Balance is a busy state, not a frozen one.' },
          { type: 'mc', prompt: 'A market with only one seller tends to stop correcting because:', choices: ['the feedback from competition is gone', 'prices are fixed by law in that case', 'buyers gradually disappear entirely', 'costs fall as the seller gets bigger'], answer: 0, hint: 'What normally pushes a price back down?', explain: 'Remove the feedback and the system stops self-correcting.' },
          { type: 'mc', prompt: 'Splitting government power across branches is most like:', choices: ['an exponent doubling repeatedly', 'a body regulating its own temperature', 'a food chain passing energy along', 'a fossil record built in layers'], answer: 1, hint: 'Which one corrects its own drift?', explain: 'Both are feedback systems that resist runaway change.' },
        ],
      },
    ],
  },
