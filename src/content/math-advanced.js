...
export const MATH_ADVANCED = [
  /* ---- m20 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'x² + 7x + 12 factors into:', choices: ['(x + 3)(x + 4)', '(x + 2)(x + 6)', '(x + 12)(x + 1)', '(x − 3)(x − 4)'], answer: 0, hint: 'Two numbers multiplying to 12 and adding to 7.', explain: '3 × 4 = 12 and 3 + 4 = 7. The pair 2 and 6 multiplies to 12 but adds to 8.' },
      ...
      { type: 'mc', prompt: 'x² − 25 factors into:', choices: ['(x − 5)(x − 5)', '(x + 5)(x − 5)', '(x + 25)(x − 1)', 'it cannot be factored'], answer: 1, hint: 'There is no middle term — which pattern is that?', explain: 'A difference of two squares: x² − 5² = (x + 5)(x − 5). Squaring (x − 5) would leave a −10x in the middle.' },
      { type: 'mc', prompt: 'Pulling the common factor out of 3x² + 12x gives:', choices: ['3(x² + 12x)', 'x(3x + 12)', '3x(x + 4)', '3x(x + 12)'], answer: 2, hint: 'What do BOTH terms share, in full?', explain: 'Both carry a 3 and an x, so 3x comes out and 3x × 4 = 12x confirms the second term.' },
  ...
  /* ---- m21 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'In 2x² − 5x + 3 = 0, the values of a, b and c are:', choices: ['2, 5, 3', '2, −5, 3', '−2, 5, −3', '2, −5, −3'], answer: 1, hint: 'The sign in front of a term belongs to that term.', explain: 'b carries its minus sign: a = 2, b = −5, c = 3. Dropping that sign is the most common slip.' },
      ...
      { type: 'mc', prompt: 'A quadratic has discriminant −7. How many times does its graph cross the x-axis?', choices: ['twice', 'once', 'never', 'it depends on a'], answer: 2, hint: 'What would you need to square to get a negative?', explain: 'No real number squares to a negative, so there are no real solutions and the curve stays entirely on one side of the axis.' },
      { type: 'mc', prompt: 'A discriminant of exactly zero means the parabola:', choices: ['crosses the axis twice', 'touches the axis at one point', 'misses the axis entirely', 'is a straight line'], answer: 1, hint: 'The ± is adding and subtracting zero.', explain: 'Both solutions collapse to the same value, so the curve meets the axis at exactly one point — its turning point.' },
  ...
  /* ---- m22 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'Simplify (x² − 4) ÷ (x − 2):', choices: ['x − 2', 'x + 2', 'x² − 2', 'it cannot be simplified'], answer: 1, hint: 'Factor the top as a difference of two squares.', explain: '(x + 2)(x − 2) ÷ (x − 2) cancels to x + 2.' },
      { type: 'mc', prompt: 'Which value is excluded from 5 ÷ (x − 7)?', choices: ['x = 5', 'x = 0', 'x = 7', 'x = −7'], answer: 2, hint: 'What makes the bottom zero?', explain: 'x = 7 makes the denominator zero, and division by zero is undefined.' },
      ...
      { type: 'mc', prompt: 'After simplifying (x² − 9) ÷ (x + 3) to x − 3, the value x = −3 is:', choices: ['now allowed, since the fraction is gone', 'still excluded', 'allowed only if x − 3 is positive', 'never relevant'], answer: 1, hint: 'Was it allowed in the ORIGINAL expression?', explain: 'The original divided by zero there. Simplifying changes how it is written, not what it was ever permitted to do.' },
    ],
  },
  ...
  /* ---- m23 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'Which sequence is exponential rather than linear?', choices: ['5, 10, 15, 20', '5, 10, 20, 40', '5, 7, 9, 11', '5, 6, 7, 8'], answer: 1, hint: 'Ask whether each step ADDS the same or MULTIPLIES by the same.', explain: 'Each term doubles. The others all add a fixed amount, which makes them linear.' },
      ...
      { type: 'mc', prompt: 'In y = a · bˣ, a value of b = 0.5 means the quantity:', choices: ['grows steadily', 'halves each step', 'stays constant', 'becomes negative'], answer: 1, hint: 'Multiplying by a half repeatedly.', explain: 'Any b between 0 and 1 gives decay. b = 0.5 halves it each step, exactly like a half-life.' },
      ...
      { type: 'mc', prompt: 'A decay curve approaches zero but never reaches it. That line is called:', choices: ['a tangent', 'an asymptote', 'a vertex', 'a radius'], answer: 1, hint: 'A line the curve gets arbitrarily close to.', explain: 'An asymptote. The value keeps halving, so it shrinks forever without ever being exactly zero.' },
    ],
  },
  ...
  /* ---- m24 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'log₁₀(1000) = 3 is another way of writing:', choices: ['10 × 3 = 1000', '10³ = 1000', '1000 ÷ 10 = 3', '3¹⁰ = 1000'], answer: 1, hint: 'The base, raised to the answer, gives the number.', explain: 'A logarithm IS an exponent. Base 10 raised to 3 gives 1000.' },
      ...
      { type: 'mc', prompt: 'Each step up the pH scale means the acidity changes by a factor of:', choices: ['1', '2', '10', '100'], answer: 2, hint: 'It is a base-10 logarithmic scale.', explain: 'Ten times per step. Two steps is a hundredfold, which is why pH 4 and pH 6 are so far apart.' },
  ...
  /* ---- m25 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      ...
      { type: 'mc', prompt: 'Which sequence is geometric?', choices: ['2, 5, 8, 11', '3, 6, 12, 24', '1, 4, 9, 16', '10, 8, 6, 4'], answer: 1, hint: 'Look for a constant multiplier, not a constant gap.', explain: 'Each term triples then doubles — specifically, each is twice the one before. The others add a fixed amount or follow the squares.' },
      ...
      { type: 'mc', prompt: 'The difference between a sequence and a series is that a series is:', choices: ['a longer sequence', 'the terms added together', 'a sequence written backwards', 'a sequence with no pattern'], answer: 1, hint: 'One is a list, the other is a total.', explain: 'The sequence is the list of terms; the series is their sum.' },
    ],
  },
  ...
  /* ---- m26 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'In a right triangle, sin of an angle equals:', choices: ['adjacent ÷ hypotenuse', 'opposite ÷ hypotenuse', 'opposite ÷ adjacent', 'hypotenuse ÷ opposite'], answer: 1, hint: 'The S in SOH CAH TOA.', explain: 'Opposite over hypotenuse. Adjacent over hypotenuse is cosine.' },
      ...
      { type: 'mc', prompt: 'Doubling every side of a right triangle changes its sine values:', choices: ['they double', 'they halve', 'they stay identical', 'they become negative'], answer: 2, hint: 'Both parts of the ratio doubled.', explain: 'Top and bottom both double, so the ratio is unchanged. That is exactly why trigonometry works at any scale.' },
      { type: 'mc', prompt: 'You know a ratio and want the angle that produced it. You use:', choices: ['the Pythagorean theorem', 'inverse trig, such as tan⁻¹', 'the quadratic formula', 'a logarithm'], answer: 1, hint: 'You need the undo of the trig function.', explain: 'Inverse trig turns a ratio back into an angle — yet another operation paired with its inverse.' },
      { type: 'mc', prompt: 'To find the height of a tower from 50 m away with an angle of 40°, you use:', choices: ['50 × sin(40°)', '50 × cos(40°)', '50 × tan(40°)', '50 ÷ tan(40°)'], answer: 2, hint: 'You know the adjacent side and want the opposite one.', explain: 'tan = opposite ÷ adjacent, so opposite = 50 × tan(40°), about 42 m.' },
    ],
  },
  ...
  /* ---- mr4 -------------------------------------------------------------- */
  {
    ...
    quiz: [
      { type: 'mc', prompt: 'Which pair are inverses of each other?', choices: ['adding and multiplying', 'exponentials and logarithms', 'factoring and adding', 'sine and cosine'], answer: 1, hint: 'Which one undoes the other exactly?', explain: 'A logarithm answers what power produced a number, which is precisely undoing an exponential.' },
      ...
      { type: 'mc', prompt: 'The sequence 2, 6, 18, 54 is:', choices: ['arithmetic, difference 4', 'geometric, ratio 3', 'arithmetic, difference 12', 'neither'], answer: 1, hint: 'Is each step adding or multiplying?', explain: 'Each term is three times the previous one, so it is geometric with ratio 3.' },
      { type: 'mc', prompt: 'A quadratic with a negative discriminant has:', choices: ['two real solutions', 'one real solution', 'no real solutions', 'infinitely many solutions'], answer: 2, hint: 'What squares to give a negative?', explain: 'No real number does, so the parabola never meets the x-axis.' },
      ...
      { type: 'mc', prompt: 'Which grows fastest for very large x?', choices: ['y = 1000x', 'y = x³', 'y = 2ˣ', 'y = x² + 500'], answer: 2, hint: 'One of these is not a polynomial.', explain: 'The exponential. It starts slowest and overtakes all of them permanently.' },
    ],
  },
];
