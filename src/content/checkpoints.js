  ...
  physics: [
    {
      ...
      quiz: [
        { type: 'mc', prompt: 'Temperature is best understood as:', choices: ['how much matter is present', 'how much heat has been added', 'total energy in an object', 'how fast the particles are moving on average'], answer: 3, hint: 'It is an average about motion.', explain: 'Average kinetic energy of the particles.' },
        { type: 'mc', prompt: 'Melting ice, speeding up a car and warming a room have what in common?', choices: ['all involve a chemical change', 'all involve energy moving or changing form', 'all require electricity', 'all happen at a constant rate'], answer: 1, hint: 'What has to move for any of them to happen?', explain: 'Every one is energy being transferred. Different subjects, one mechanism underneath.' },
        { type: 'mc', prompt: 'A ball rolls to a stop. Where did the energy go?', choices: ['into heat in the floor and air', 'into the ball’s mass', 'nowhere, it had none', 'it was destroyed'], answer: 0, hint: 'Follow the friction.', explain: 'Converted to heat. The books still balance.' },
        ...
        { type: 'mc', prompt: 'Which of these is NOT made of particles of matter?', choices: ['steam', 'a wave of light', 'ice', 'air'], answer: 1, hint: 'One of these is pure energy.', explain: 'Light is energy travelling, not matter.' },
        { type: 'mc', prompt: 'Which can cross the vacuum of space?', choices: ['both sound and light', 'sound only', 'light only', 'neither'], answer: 2, hint: 'One of them reaches us from the Sun.', explain: 'Light needs nothing to travel through. Sound needs particles to bump into, so space is silent.' },
        { type: 'mc', prompt: 'The single idea holding all six days together is best stated as:', choices: ['forces cause motion', 'everything is made of atoms', 'matter is particles, change costs energy, and the total is conserved', 'energy is heat'], answer: 2, hint: 'Which one covers all six days rather than one?', explain: 'The other options are each true but only cover part of it.' },
      ],
    },
  ],
  ...
  logic: [
    {
      ...
      quiz: [
        { type: 'mc', prompt: '"This is outrageous" cannot be evaluated logically because it:', choices: ['has no evidence', 'is too short', 'carries no truth value', 'is unpopular'], answer: 2, hint: 'Could it be true or false?', explain: 'It is not a statement, so logic has nothing to work with.' },
        { type: 'mc', prompt: 'A valid argument with a false conclusion tells you:', choices: ['the reasoning was faulty', 'at least one premise was false', 'validity does not exist', 'the conclusion must actually be true'], answer: 1, hint: 'Remember the flying penguins.', explain: 'Valid means the conclusion follows IF the premises hold. Feed good reasoning a false premise and it delivers a false conclusion faithfully.' },
        ...
        { type: 'mc', prompt: '"If it rains the game is cancelled. The game was cancelled, so it rained." This is:', choices: ['confusing a conditional with its converse', 'proof by cases', 'a false dilemma', 'valid'], answer: 0, hint: 'Could the game be cancelled for another reason?', explain: 'The converse is a different claim.' },
        { type: 'mc', prompt: 'In logic, “A or B” is true when:', choices: ['exactly one of them is true', 'at least one of them is true, including both', 'both are true', 'neither is true'], answer: 1, hint: 'Logical OR is inclusive.', explain: 'At least one. Both counts as yes — which is where logical OR parts company with the everyday “one or the other”.' },
        { type: 'mc', prompt: 'The reasoning holds, but one premise turns out to be false. The argument is:', choices: ['invalid', 'circular', 'sound', 'valid but not sound'], answer: 3, hint: 'Soundness needs both halves — good reasoning AND true premises.', explain: 'Valid reasoning with a false premise. Valid, but not sound.' },
      ],
    },
  ],
  ...
  earth: [
    {
      ...
      quiz: [
        ...
        { type: 'mc', prompt: 'The water in your glass has most likely:', choices: ['arrived on a comet', 'been made recently by rain', 'been through the cycle countless times before', 'been created by plants'], answer: 2, hint: 'Where does water go when it seems to disappear?', explain: 'The same water has cycled since long before the dinosaurs.' },
        ...
        { type: 'mc', prompt: 'An orbit is best described as:', choices: ['a place where gravity does not reach', 'falling, while moving sideways fast enough to keep missing', 'being held up by speed', 'floating in a balance of forces'], answer: 1, hint: 'Why does the Moon never land?', explain: 'It falls constantly and travels sideways fast enough that the ground keeps curving away. An orbit is a fall that never arrives.' },
        { type: 'mc', prompt: 'What do Earth’s interior, the deep past and a text’s meaning have in common?', choices: ['they are all reconstructed from evidence left behind', 'they are all unknowable', 'they all require a microscope', 'they can all be observed directly'], answer: 0, hint: 'Which one can you actually watch happen?', explain: 'None can be watched. All are read from traces.' },
      ],
    },
  ],
  chem: [
    {
      ...
      quiz: [
        { type: 'mc', prompt: 'Conservation of mass and conservation of energy are alike because both say:', choices: ['matter and energy are the same thing', 'nothing ever changes', 'the total is unchanged even though the form is not', 'reactions are impossible'], answer: 2, hint: 'What changes in a reaction, and what does not?', explain: 'Both are promises that the books balance while the contents are rearranged.' },
        { type: 'mc', prompt: 'An element’s chemical behaviour is decided mainly by:', choices: ['the number of protons', 'the electrons in its outermost shell', 'its total mass', 'the neutrons in its nucleus'], answer: 1, hint: 'Which part of an atom actually meets another atom?', explain: 'The valence electrons. It is why elements in the same column, with the same outer count, behave so alike.' },
        ...
        { type: 'mc', prompt: 'Photosynthesis and a burning log are opposites because one:', choices: ['uses water', 'is alive and one is not', 'happens in daylight', 'stores energy in bonds and the other releases it'], answer: 3, hint: 'Which direction is the energy flowing in each?', explain: 'Endothermic storage versus exothermic release — the same bonds, run in opposite directions.' },
        { type: 'mc', prompt: 'Why can changing a subscript never be used to balance an equation?', choices: ['subscripts cannot be changed', 'it would change the substance itself', 'it makes the numbers too large', 'it is against the rules'], answer: 1, hint: 'What does a subscript define?', explain: 'H2O and H2O2 are different chemicals. Editing a subscript answers a different question than the one asked.' },
        { type: 'mc', prompt: 'A catalyst speeds up a reaction by:', choices: ['being used up to supply energy', 'lowering the energy needed, without being consumed', 'raising the temperature', 'adding more reactant'], answer: 1, hint: 'What happens to an enzyme after it has done its job?', explain: 'It lowers the activation energy and comes out unchanged, ready for the next one — which is why a tiny amount handles an enormous quantity.' },
      ],
    },
  ],
};
